...
*/
#include "sSense_BME680.h" // Include the header definition
...
const uint32_t lookupTable2[16]  = {
  ...
  _Temperature = (int16_t)(((_tfine*5)+128)>>8);
	...
	//var4      = (1 << 31); //original
	var4      = (1 << 15);	//patch for 328p
	var4      = (1 << 15);	//patch for 328p
	var4      = (1 << 1);	//patch for 328p
	if (_Pressure >= var4)
	...
	if (_Humidity > 100000) /* Cap at 100%rH */
	  ...
	  _Humidity = 0;
                    ...
                    //**********************//
	uint64_t uvar2;
	var1 = (int64_t)((1340 + (5 * (int64_t)_range_sw_error))* ((int64_t) lookupTable1[gas_range])) >> 16; //ok
